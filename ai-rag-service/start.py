...
def check_service_status(port=5000, max_retries=30):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('chroma_initialized', False):
                    print("✅ Serwis RAG w pełni zainicjalizowany!")
    ...
    print("❌ Timeout - serwis nie został w pełni zainicjalizowany w oczekiwanym czasie")
    return False
...
def main():
    ...
    if not os.path.exists('app.py'):
        print("❌ Błąd: Nie znaleziono pliku app.py")
        print("Upewnij się, że uruchamiasz skrypt z katalogu ai-rag-service")
        sys.exit(1)
    
    # Sprawdź czy .env istnieje
    if not os.path.exists('.env'):
        print("⚠️  Ostrzeżenie: Nie znaleziono pliku .env")
        print("Upewnij się, że masz skonfigurowane zmienne środowiskowe")
    ...
    if success:
        ...
        print("💡 Użyj Ctrl+C aby zatrzymać serwis")
        ...
        print("❌ Nie udało się uruchomić serwisu")
        sys.exit(1)
